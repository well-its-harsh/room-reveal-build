    ...
    <div className="container py-6 md:py-12 max-w-3xl">
      ...
      <div className="space-y-4">
        {items.map((item) => (
          ...
          >
            <img src={item.product.image} alt={item.product.name} className="w-20 h-20 rounded object-cover bg-secondary" />
            <div className="flex-1 min-w-0">
              <h3 className="font-heading text-sm font-medium text-foreground line-clamp-1">{item.product.name}</h3>
              <p className="text-sm text-accent font-semibold font-body">₹{item.product.price.toLocaleString('en-IN')}</p>
              <div className="flex items-center gap-2 mt-2">
                <button onClick={() => updateQuantity(item.product.id, item.quantity - 1)} className="w-7 h-7 rounded bg-secondary flex items-center justify-center text-foreground hover:bg-muted">
      ...
      <div className="mt-8 p-6 bg-secondary rounded-lg">
        ...
        <Button
          ...
          onClick={() => toast.info("Checkout coming soon! For now, please contact us to place your order.")}
        >
          Proceed to Enquiry
        </Button>
